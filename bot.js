...
let rol = member.guild.roles.cache.find(r => r.name === "Prisoner");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('798562334082859026');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('798562334082859024');
  }, ms(süre));
}
})
...
data.set(`${member.guild.id}.jail.${kişi.id}.roles.${r.id}`, r.id )})
  ...
  .setColor(`RANDOM`)
  .setDescription(`Çık gir yaparak jailden çıkabileceğini mi sandın :D?`)
  .setTimestamp()
    member.send(wasted)
} 
          ...
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                      ...
                      return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu sunucuda küfür filtresi etkin.`).setColor('RANDOM').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
          ...
          try {
            if (!oldMessage.member.hasPermission("BAN_MEMBERS")) {
                  oldMessage.delete();
                      ...
                      return oldMessage.channel.send(new Discord.MessageEmbed().setDescription(`${oldMessage.author} Bu sunucuda küfür filtresi etkin.`).setColor('RANDOM').setAuthor(oldMessage.member.displayName, oldMessage.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
 ...
 if(!db.has(`reklam_${msg.guild.id}`)) return;
        ...
        if (reklam.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("BAN_MEMBERS")) {
                  msg.delete();
                    return msg.channel.send(new Discord.MessageEmbed().setDescription(`${msg.author} Bu sunucuda reklam filtresi etkin.`).setColor('RANDOM').setAuthor(msg.member.displayName, msg.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));
