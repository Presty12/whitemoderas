...
//-------------------------------------------------------------------------------\\
  if(!["798562334137778198", "798562334137778204"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR"))) 
...
message.channel.send(new MessageEmbed().setColor('RANDOM').setDescription(`${message.author} Sana Ait Sicil Verilerini Sildim!`))
}
  
if(member) {
let sicil = kdb.delete(`kullanici.${member.id}.sicil`) || [];
message.channel.send(new MessageEmbed().setColor('RANDOM').setDescription(`${member} Kullanıcısına Ait Sicil Verilerini Sildim!`))
